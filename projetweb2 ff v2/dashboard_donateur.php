...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['donate'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM projet WHERE id_projet = ? AND date_limite >= CURDATE() AND montant_total_collecte < montant_total_a_collecter");
    ...
    if ($project) {
        ...
        if ($amount > 0 && $amount <= $remaining) {
            try {
                ...
                $insert_query = "INSERT INTO donateur_projet (id_projet, id_donateur, montant_participation, date_participation) VALUES (?, ?, ?, NOW())";
                $stmt1 = $pdo->prepare($insert_query);
                $stmt1->execute([$project_id, $donor_id, $amount]);

                // 2. Update project collected amount
                $update_query = "UPDATE projet SET montant_total_collecte = montant_total_collecte + ? WHERE id_projet = ?";
...
$base_query = "SELECT * FROM projet WHERE date_limite >= CURDATE() AND montant_total_collecte < montant_total_a_collecter";
...
$donation_history = [];
$history_query = "SELECT p.titre, p.description, SUM(dp.montant_participation) as total_don, COUNT(dp.id) as nombre_dons 
                  FROM donateur_projet dp
                  JOIN projet p ON dp.id_projet = p.id_projet
                  WHERE dp.id_donateur = ?
                  GROUP BY p.id_projet";
$stmt = $pdo->prepare($history_query);
$stmt->execute([$donateur['id']]);
